...
async def roll_1d100():
    tens = random.randrange(0,10)
    ones = random.randrange(0,10)
    
    if tens == 0 and ones == 0:
        d100= 100
    else:
        d100 = int(str(tens)+str(ones))

    return(d100)


async def get_results(bonus,number):
    if number >= 1:
        no_and_threshhold = 10 - bonus
        no_threshhold = 30 - bonus*3
        no_but_threshhold = 50 - bonus*4
        yes_but_threshhold = 70 - bonus*4
        yes_threshhold = 90 - bonus*3
    else:
        no_and_threshhold = 10 + bonus*3
        no_threshhold = 30 + bonus*4
        no_but_threshhold = 50 + bonus*4
        yes_but_threshhold = 70 + bonus*3
        yes_threshhold = 90 + bonus

    if number < no_and_threshhold:
        result= "no, and"
    elif number < no_threshhold:
        result = "no"
    elif number < no_but_threshhold:
        result = "no, but"
    elif number < yes_but_threshhold:
        result = "yes, but"
    elif number < yes_threshhold:
        result = "yes"
    else:
        result = "yes, and"
...
#a function to print customized outputs from a list provided ahead of time by the players

async def declare_results(result):
#     if result == "no, and":
#         custom_results()
#     elif result == "no":
#         custom_results()
